class RealTimeChart {
    ...
    if (chckclmn==true)
    {
        ntx1 = '<td width="40px"><input type="checkbox" checked onchange="'+iname+'.maincheckchange()"></td>';
    }
   if (clrclmn==true)
    {
  ...
  //<div style="width: 100%;height: 100%;margin: 0;padding: 0;overflow: hidden;position:relative"></div>
   ...
   //canvhvr.ontouchmove = "dsaw(event)";
        ...
        this.maincheckchange = function()
        {
            var xws = tbinput[0].checked;
            for (var i = 1; i < tbinput.length ; i++)
            {
                tbinput[i].checked = xws;
            }
            drawpoints();
        };
